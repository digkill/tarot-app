...
export const startYooKassaCheckout = async (productId: string): Promise<CheckoutStatus> => {
    const session = await createCheckoutRequest(productId);
    await WebBrowser.openAuthSessionAsync(session.checkoutUrl, 'mediarisetarot://billing/complete');
